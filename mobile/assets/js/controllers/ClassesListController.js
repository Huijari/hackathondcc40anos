						...
						Class.getById(classes[key].id).on('value', function(snapshot) {
							...
							if(classe) {
								...
								classe.dia_semana.forEach(function(dia) {
									if(dias[dia] == moment().day() && moment({hour: classe.hora_inicial.split(':')[0], minute: classe.hora_inicial.split(':')[1]}) <= moment() && moment() <= moment({hour: classe.hora_final.split(':')[0], minute: classe.hora_final.split(':')[1]})) {
